...
class Agent():
    ...
    def _init_buffer(self):
        ...
        self.observations_t     = torch.zeros((self.envs_count, ) + (1, ) + self.observation_shape).to(self.model.device)
        self.im_b               = torch.zeros((self.envs_count, self.batch_size, self.actions_count)).to(self.model.device)
    ...
    def process_env(self, env_id = 0):
        ...
        observation_prev_t          = self.observations_t[env_id].clone()
        self.observations_t[env_id] = observation_t

        logits, value, im   = self.model.forward(observation_t, observation_prev_t)
        ...
        if self.enabled_training:
            ...
            self.im_b[env_id][self.idx]         = im.squeeze(0)
    ...
    def compute_loss(self, env_id = 0):
        ...
        L = (target_action - predicted_actions)**2
        '''
        loss_im  = self._loss_im(env_id)
        loss_im  = self.im_beta*loss_im.mean()

        #final loss to minimize
        loss = loss_value + loss_policy + loss_entropy + loss_im
        return loss
    ...
    def _loss_im(self, env_id = 0):
        loss   = 0.0
        target = torch.zeros(self.batch_size, self.actions_count)
            
        for i in range(self.batch_size-1):
            action = self.action_b[env_id][i]
            target[i+1][action] = 1.0

        target = target.to(self.model.device)
        
        loss = (target - self.im_b[env_id])**2
