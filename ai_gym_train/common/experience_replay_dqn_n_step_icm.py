...
class Buffer():
    ...
    def get_random_batch(self, batch_size, device):
        ...
        state       = torch.zeros(state_shape,  dtype=torch.float32).to(device)
        state_next  = torch.zeros(state_shape,  dtype=torch.float32).to(device)

        q_values_target = torch.zeros(q_values_shape,  dtype=torch.float32).to(device)
        actions_one_hot = torch.zeros(q_values_shape,  dtype=torch.float32).to(device)

        for i in range(0, batch_size):
            n      = numpy.random.randint(self.length() - self.steps - 2)
            ...
            state[i]        = torch.from_numpy(self.buffer[n].observation).to(device)
            state_next[i]   = torch.from_numpy(self.buffer[n+1].observation).to(device)

            q_values_target[i] = torch.from_numpy(q_values).to(device)
            actions_one_hot[i][action] = 1.0 
            
        return state, state_next, q_values_target, actions_one_hot
