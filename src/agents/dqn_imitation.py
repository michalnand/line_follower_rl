...
class Agent():
    def __init__(self, env, model, config, expert_model, save_path = None, save_stats = True):
        self.env = env
        ...
        self.expert_decay   = config.expert_decay
    ...
    def train_model(self):
        #obtain state and target q values from replay buffer
        input, q_target = self.experience_replay.get_random_batch(self.batch_size, self.model.device)

        #imitation RL :

        #obtain expert output
        q_expert_target = self.expert_model.forward(input)

        
        #fill target with expert values, probability depends on expert_decay value
        v = self.expert_decay.get()

        for i in range(self.batch_size):
            if numpy.random.rand() < v:
                q_target[i] = q_expert_target[i]
        
        #continue with common deep Q net
